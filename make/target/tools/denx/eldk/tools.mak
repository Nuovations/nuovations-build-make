...
#      flags for tools common to all Denx Software Engineering Embedded
#      Linux Development Kit (ELDK) versions.
#

#
# The ELDK has historically used and probably will continue to use GNU
# tools for its tool chain, so just source the GNU GCC common tools.
#

include target/tools/gnu/gcc/tools.mak

#
# However, the ELDK is almost exclusively for cross-compiling, so
...
GccSedBuild		= .\{1,\}
GccSedRegExp		= ^.*$(GccSedTool) (GCC) \($(GccSedVers)\)\( $(GccSedBuild)\)*.*$$
GccSedCommand		= "s/$(GccSedRegExp)/\1 \3/gp"
...
BinutilsSedBuild	= [[:digit:]]\{1,\}
BinutilsSedRegExp	= ^.*GNU $(BinutilsSedTool) \((.\{1,\}) \)*\($(BinutilsSedVers)\) *\($(BinutilsSedBuild)\)*.*$$
