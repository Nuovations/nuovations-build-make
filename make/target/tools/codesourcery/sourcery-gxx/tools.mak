...
#      flags for tools common to all CodeSourcery Sourcery G++ versions.
#

#
# Sourcery G++ has historically used and probably will continue to use
...
#
# However, Sourcery G++ is almost exclusively for cross-compiling, so
...
GccSedBuild		= .\{1,\}
GccSedRegExp		= ^.*$(GccSedTool) (Sourcery G++ \(Lite \)*\($(GccSedBuild)\)) \($(GccSedVers)\)
...
BinutilsSedBuild	= .\{1,\}
BinutilsSedRegExp	= ^.*GNU $(BinutilsSedTool) (Sourcery G++ \(Lite \)*\($(BinutilsSedBuild)\)) \($(BinutilsSedVers)\)
