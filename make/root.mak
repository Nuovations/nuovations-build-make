...
#
# Clean all dependencies and intermediate object files generated by
# Nuovations Build (Make).  deleting what is contained in nested
# .build and .depend directories.  This is different than
# product configuration-specific distclean, which recursively calls distclean
# in submakefiles.  It might be good to run the
# distclean-product-config target first because the local Makefiles
# might do more cleanup (like removing files untarred in build steps)
# than what this rule alone does.
#

.PHONY: distcleanall
distcleanall:
	$(Echo) "DistCleaning in \"$(call GenerateBuildRootEllipsedPath,$(ResultBaseDirectory))\""
	-$(Verbose)$(RM) $(RMFLAGS) -r "$(ResultBaseDirectory)"
	$(Echo) "DistCleaning in \"$(call GenerateBuildRootEllipsedPath,$(BuildRoot))\""
	-$(Verbose)find "$(BuildRoot)" -depth -type d \( -name "$(BuildBaseDirectory)" -o -name "$(DependBaseDirectory)" \) -exec $(RM) $(RMFLAGS) -r {} \;
